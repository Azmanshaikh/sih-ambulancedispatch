<script lang="ts">
  ...
  const interpolateRoute = (fromLat: number, fromLng: number, toLat: number, toLng: number, steps = 12) => {
    ...
    pts[0] = [fromLat, fromLng];
    pts[pts.length - 1] = [toLat, toLng];
    return pts;
  };

  async function handleSimulateDispatch() {
    dispatchStatus = 'Calculating Route...';
    ...
    try {
      ...
      if (result.status === 'success' && result.data) {
        ...
        const isRealRoute = routeCoords && routeCoords.length > 1 && Math.abs(routeCoords[0][0]) > 1;
        const amb = AMB_LOCATIONS.find(a => a.id === ambulance_id) || AMB_LOCATIONS[0];
        route = isRealRoute ? routeCoords : interpolateRoute(amb.lat, amb.lng, incidentLat, incidentLng);
        markers = [
        ...
        dispatchStatus = `🚑 ${ambulance_id} dispatched — ETA ${Math.round((eta_seconds || 480) / 60)} min`;
      } else {
      ...
      route = interpolateRoute(amb.lat, amb.lng, incidentLat, incidentLng);
      markers = [
        { position: [incidentLat, incidentLng], popup: '🚨 Emergency Incident', type: 'incident' },
        { position: [amb.lat, amb.lng],          popup: '🚑 AMB-101 (offline mode)', type: 'ambulance' },
      ];
      dispatchStatus = '🚑 AMB-101 — Offline Mode (ETA ~8 min)';
    }
  }
</script>
...
<div class="h-full flex flex-col" style="overflow: hidden;">
  <div class="p-5 grid grid-cols-12 gap-5 h-full overflow-hidden">
    ...
    <div class="col-span-3 flex flex-col gap-4 overflow-y-auto no-sb pb-3">
      <section class="bg-slate-900/60 p-5 rounded-xl border border-slate-800/60 flex-shrink-0">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <div class="text-2xl font-black text-white">—<span class="text-xs text-slate-500 ml-1">min</span></div>
            <div class="text-[9px] uppercase tracking-wide text-slate-400 mt-0.5">Avg Response</div>
          </div>
          <div>
            <div class="text-2xl font-black text-white">—<span class="text-xs text-slate-500 ml-1">units</span></div>
      ...
      <section class="flex flex-col gap-3 flex-1 min-h-0">
        ...
        <div class="space-y-2.5 overflow-y-auto no-sb flex-1">
          <button 
          onclick={handleSimulateDispatch}
            class="w-full py-2.5 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white rounded-lg text-xs font-black uppercase tracking-wider transition-all shadow-lg shadow-red-900/40 active:scale-95"
          >
            🚨 Initiate Dispatch
          </button>
        </div>
    ...
    <div class="col-span-6 relative rounded-2xl overflow-hidden shadow-2xl border border-slate-800">
      <MapWidget id="dash-map" {route} {markers} />
        ...
        <div class="flex justify-between items-start pointer-events-auto">
          <div class="glass px-4 py-2 rounded-xl border border-slate-700/50 shadow">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs font-bold tracking-widest uppercase text-white">Live Sector Feed</span>
            </div>
          </div>
          <div class="glass px-3 py-1.5 rounded-xl border border-yellow-500/30 text-[10px] text-yellow-400 font-bold uppercase tracking-widest">
            🛰️ Locating…
          </div>
        </div>
      </div>
    ...
    <div class="col-span-3 flex flex-col gap-4 overflow-y-auto no-sb pb-3">
      <section class="bg-gradient-to-br from-slate-900 to-blue-900/20 p-5 rounded-2xl border border-blue-500/20 shadow-xl flex-shrink-0">
        ...
        <div class="flex gap-3">
          ...
          <div>
            <p class="text-xs font-bold text-white">Fleet Optimal</p>
            <p class="text-[10px] text-slate-400 mt-1">All sectors covered. Response times within protocol.</p>
          </div>
        </div>
      </section>
          ...
          <div class="relative">
            ...
            <div class="bg-slate-900/40 p-3 rounded-xl border border-slate-800/50">
              <div class="flex justify-between items-center mb-1">
                <span class="text-[10px] font-bold text-blue-500 uppercase">System</span>
